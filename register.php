...
$query="select * from branch_course";
...
if($result = mysqli_query($conec, $query))
{
  ...
  if(mysqli_num_rows($result) < 1){
      header("location: index.php?Error_check_later");
  }
...
<form class="form-horizontal" method="POST" action="pay.php" enctype="multipart/form-data" style="margin-left:220px; width: 950px;">
    ...
    <div class="form-group">
            ...
            <div class="col-md-4">
              ...
              while ($row = mysqli_fetch_array($result)) {
                  ...
                  echo "<option data-value='{'course':{$course[$i]}, 'fee':{$fee[$i]} }'>{$course[$i++]}</option>";
              }
          ...
          echo "<input type='number' class='form-control' id='Price' name='amount' value='' readonly>";
    ...
    <div class="form-group">
            <label class="col-md-4 control-label" for="selectbasic">Branch Name</label>
            <div class="col-md-4">
              <select id="branch_code" name="branch_code" class="form-control">
                    <?php
                      ...
                      $result = mysqli_query($conec, $query);
                        ...
                        while($row = mysqli_fetch_array($result)) {
                          $branch[] = $row['branch'];
                          echo "<option value={$branch[$j]}> {$branch[$j++]} </option>";
                        }
                    ?>
               </select>
             </div>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script> <!-- CHANGE THE JQUERY FILE DEPENDING ON THE VERSION YOU HAVE DOWNLOADED -->

<script type="text/javascript">

  $(document).ready(function(){ /* PREPARE THE SCRIPT */
    $("#selectbasic").change(function(){ /* WHEN YOU CHANGE AND SELECT FROM THE SELECT FIELD */
      var course = $(this).val(); /* GET THE VALUE OF THE SELECTED DATA */
      var dataString = "course="+course; /* STORE THAT TO A DATA STRING */

      $.ajax({ /* THEN THE AJAX CALL */
        type: "POST", /* TYPE OF METHOD TO USE TO PASS THE DATA */
        url: "show_fee.php", /* PAGE WHERE WE WILL PASS THE DATA */
        data: dataString, /* THE DATA WE WILL BE PASSING */
        success: function(result){ /* GET THE TO BE RETURNED DATA */
        $("#Price").val(result); /* THE RETURNED DATA WILL BE SHOWN IN THIS DIV */
        // alert(result);
        }
      });
